...
private:
	...
	time_t login_time =0;//所有有关记录的函数加上这个量！
...
private:
	...
	char administrator_id[30] = "\0";//用于登记管理员id

	char password[30] = "\0";//登记输入的密码

	char check_code[30] = "\0";//登记校验码（重置密码）
...
char* showtime(void);//该函数用于获取当地时间
...
char* showtime(void)//获取当地时间
{
	time_t currenttime;//定义用于储存时间的变量
	char shijian[26];
	time(&currenttime);//获取当地时间，但是输出类型是long long int
	ctime_s(shijian, sizeof(shijian), &currenttime);//转化为char
	return shijian;//输出时间
}
...
{
	char currenttime[26];
	ctime_s(currenttime, sizeof(currenttime), &login_time);
	return currenttime;
}
...
{
	strcpy_s(iferror, strlen(iferror) + 1, if_error);
}

void computer::change_ifuse(const char* if_use)//修改链表中的ifuse
{
	strcpy_s(ifuse, strlen(ifuse) + 1, if_use);
}
...
{	
	...
	do
	{
		...
		for (int i = 1; i <= N; i++)
		{
			if (p3->get_computer_id() == computer_id)
			{
				if (!strcmp(p3->get_ifuse(), "No"))//没有人使用电脑,则无需登出
				{
					cout << "该电脑没有人使用！" << endl;
					cout << "请重新输入编号……" << endl << endl;
					computer_id = 0;//这里充当sign的功能，其实就是从头重新执行一次
...
void administratorsystem::administrator_create(void)//这是根据文件里面的内容读取账号信息并重新创建出来
{
	...
	file.seekg(-1, ios::cur);//将光标移动到N前面
	file >> N;
	...
	for(int i=0;i<5*N;i++)//有N个链表待初始化，一个链表初始化5次，因此共5*N
	{
...
void administratorsystem::reset_password(void)//用于重置密码
{
	...
	while (file1 >> password_txt[0])//不使用eof是因为eof会多读取一次
	{
		for (int l = 0; l < 30; l++)
		{
			file1 >> password_txt[l];
...
void administratorsystem::show_computer(void)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		if (!strcmp(p1->get_iferror(), "true"))//iferror为true,也就是电脑有故障
			...
			cout << "[使用]"<<"||" << "       " << p1->get_id() << "||" << "       " << p1->get_name() << endl << endl;
		else//电脑无故障且没有被使用
			cout << "[空闲]"<<"||" << endl << endl;
		p1 = p2;
		if(p1!=NULL)
